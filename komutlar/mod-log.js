...
let logkanal = await db.fetch(`nordxmodlog${message.guild.id}`)
  ...
  if (args[0] === "sıfırla" || args[0] === "kapat") {
   ...
   message.channel.send(new Discord.MessageEmbed().setDescription(`ModLog Kanalı başarıyla sıfırlandı`)             .setColor('#ff0000')
);

    return
  }
  
if (!logk) return message.channel.send(new Discord.MessageEmbed().setDescription(`Bir modlog kanalı belirt`)             .setColor('#ff0000')
);
 

db.set(`nordxmodlog${message.guild.id}`, logk.id)

message.channel.send(new Discord.MessageEmbed().setDescription(`Mod-Log kanalı başarıyla ${logk} olarak ayarlandı`)             .setColor('#ff0000')
);
